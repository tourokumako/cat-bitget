...
- **1m足への回帰提案（全Priority共通）**：1m MACD はエッジゼロ確認済み・手数料負け確定。全Priority を5m足で運用する方針。1m足回帰は禁止。
...
3. 設計思想（5m足・Priority特性別最適化）と整合しているか → 非整合なら提案しない
```
...
- **全Priority: 5m足で統一**（1m足はエッジゼロ確認済み・手数料負け確定のため廃止）
- P2/P23: スキャル型・5m足（TP=0.005で調整中）
- P4/P22/P24: スイング型・5m足（TP幅・保有時間はPriority単位で調整可）
- P1/P21: 新規・5m足・MACD(12,26,9)クロス・TRAIL_EXIT設計
