...
### 本番成績分析（3/29 15:00以降、26件）

- net合計: **-13.28 USD**（手動クローズ分+16.77含む）
- MAE_CUT 1件（-99 USD）+ MIDTERM_CUT 2件（-79 USD）が主因
- TIME_EXITは3番目の要因（-46.8 USD）。延長より add枚数制限の見直しが有効
- TP利確は17件・機能は正常。サンプル数がまだ少なく統計的判断は困難

### 45135バグ（2026-03-31）の影響確認

- SHORT @67639.5 のTP設定時に45135エラー → state未作成 → reconciliation STOP×3
- ユーザーが手動クローズ（@66903.1、+16.77 USD）
- live_trades.csv に MANUAL_CLOSE として追記済み ✅
- バグ自体は 4c302d3 コミットで修正済み

### 現在のポジション（2026-04-01 時点）
- SHORT 0.0005 BTC @ 66886.2、TP=66650.8（TTL切れ部分約定残り）
- LONG なし

### 検討中の改善案（未着手・バックテスト要）
- P24_RSI_MIN 緩和（65→60）でP24発火率向上の検討
- P4/P2 の add枚数上限見直し（MIDTERM_CUT/MAE_CUT の大損対策）
- マルチタイムフレーム（1時間足）は規模大・バックテスト全体やり直しが必要なため保留
