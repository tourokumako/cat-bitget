#!/usr/bin/env python3
"""
analyze_signal_n3_exit.py — N3 (ADX Spike) シグナルの Exit 再設計仮想シミュ

前提: N3 素エッジ 365d で 勝率 66.11% / $-1.07/dt-day / 5.9件/dt-day（TP/SL非対称で赤字）
本スクリプトは TP_PCT / SL_PCT / MAX_HOLD の組合せを 365d で走査する。
180d を使わず 365d 直接走査（N1の過学習教訓）。
...
Usage:
  python3 scripts/analyze_signal_n3_exit.py data/BTCUSDT-5m-2025-04-01_03-31_365d.csv
"""
...
def main(csv_path: str):
    print(f"[N3-exit] CSV: {csv_path}")
    df, fire_idx, dt_days = load_signals(csv_path)
    print(f"[N3-exit] 発火: {len(fire_idx)}  DT日数: {dt_days}")
    ...
    if final["winrate"] >= 60.0 and final["dt"] >= 10.0:
        print("  ✅ 採用基準クリア → 本実装フェーズ")
    elif final["dt"] >= 5.0:
        print("  △ 基準未達だが$5/dt-day超 → 追加シグナル条件強化余地")
    else:
        print("  ❌ 全組合せで$10/dt-day未達 → N3 完全却下 → 次候補へ")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/analyze_signal_n3_exit.py <csv_path>")
        sys.exit(1)
    main(sys.argv[1])
