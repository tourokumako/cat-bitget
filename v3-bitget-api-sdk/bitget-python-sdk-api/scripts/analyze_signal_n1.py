#!/usr/bin/env python3
"""
analyze_signal_n1.py — N1 (EMA20 Rejection SHORT) シグナル検証

新規Priority設計フロー Step 2: フィルターなし・タイトなTP/SLで素の勝率を確認する。

シグナル条件（DT限定）:
  high[i]  >  ema_20[i]     # 一度タッチ
  close[i] <= ema_20[i]     # リジェクト確定
  close[i] <  open[i]       # 陰線
  bb_mid_slope[i] < 0       # 下降局面
  date(i) is DOWNTREND      # 日足MA70判定
...
Entry/Exit:
  ...
  TIME  : 48本（240分）
  SIZE  : 0.024 BTC
  FEE   : MAKER 0.00014 片道 × 2

採用基準:
  勝率 >= 60% かつ $/dt-day >= $10 → 本実装フェーズ
  勝率 >= 60% かつ $/dt-day < $10  → フィルタ追加余地あり
  勝率 < 60%                       → N1 却下 → N3 ピボット

Usage:
  python3 scripts/analyze_signal_n1.py data/BTCUSDT-5m-2025-10-03_04-01_combined_180d.csv
"""
...
def simulate_trade(df: pd.DataFrame, entry_idx: int):
    ...
    for j in range(entry_bar, entry_bar + max_hold + 1):
        ...
        # 同一bar内 TP/SL 同時 hit は SL 優先（保守的）
        if high_j >= sl_price:
...
def main(csv_path: str) -> None:
    ...
    if winrate >= 60.0 and net_per_day >= 10.0:
        print(f"  OK 勝率 {winrate:.1f}% >=60% かつ $/dt-day ${net_per_day:.2f} >=$10 -> 本実装フェーズ")
    elif winrate >= 60.0:
        print(f"  △ 勝率 {winrate:.1f}% >=60% だが $/dt-day ${net_per_day:.2f} <$10 -> フィルタ追加余地あり")
    else:
        print(f"  NG 勝率 {winrate:.1f}% <60% -> N1 却下 -> N3 へピボット")


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/analyze_signal_n1.py <csv_path>")
        sys.exit(1)
    main(sys.argv[1])
