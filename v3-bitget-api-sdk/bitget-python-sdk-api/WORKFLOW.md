...
Step 2.5. 設計思想チェック ← STOP（提案前に必ず通過すること）
  以下を全て満たす案のみ Step 3 に進める。1つでも NG なら案を捨てて Step 2 に戻る。

  □ TP率が現状以上を維持するか？
      NG 例: TP幅を広げる → TP率低下 → 「手堅く稼ぐ・確実性重視」に反する
  □ TIME_EXIT 件数が増えないか？
      NG 例: TP幅を広げる → 到達できずTIME_EXITへ → 損失拡大
  □ 平均保有時間が24h以内に収まるか？
      NG 例: TIME_EXIT_MIN を極端に延ばす → 設計対象外の保有期間
