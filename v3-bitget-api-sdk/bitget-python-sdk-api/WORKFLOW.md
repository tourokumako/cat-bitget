...
| シミュレーター | `runner/replay_1m.py`（5m対応済み） |
...
| 項目 | 値 |
|------|-----|
| ロング条件 | RSI(9) < 30 AND Stoch K/Dクロス上抜け |
| ショート条件 | RSI(9) > 70 AND Stoch K/Dクロス下抜け |
| TP | 0.15% |
| SL | 0.30% |
| Stoch K/D/Slowing | 9/3/3 |

### Replay 結果履歴（TIME_EXIT廃止後）

#### 1m足（全シグナル）

| シグナル | TP率 | ランダム理論値 | NET/日 |
|------|------|------|------|
| mean_reversion | 63.6% | 66.7% | -$4.55 |
| trend_follow | 31.9% | 33.3% | -$29.55 |
| ema20 | 63.6% | 66.7% | -$13.89 |
| rsi9 | 65.1% | 66.7% | -$6.05 |
| Stoch+RSI(30/70) | 64.0% | 66.7% | -$33.48 |
| **全て1m足ではランダム以下** | | | |

#### 5m足（Stoch+RSI）

| 設定 | TP率 | ランダム理論値 | GROSS | NET/日 |
|------|------|------|------|------|
| TP=0.30%, SL=0.15% | 32.2% | 33.3% | - | -$8.25 |
| **TP=0.15%, SL=0.30%** | **67.7%** | **66.7%** | **+$205** | **-$5.56** |

**5m足でランダムを初めて上回った。GROSSが初のプラス。**

### 現状の課題（2026-04-07）

- 損益分岐TP率: **72.8%** 必要（現在67.7%、あと5.1%）
- 手数料が$1,212/180日と大きい（件数2,073件 × $0.585/件）

### 次のタスク

- [ ] RSI閾値を厳しくする（30→25 or 20）でTP率72.8%超を狙う
- [ ] TP率改善後、90日データでも検証（過学習チェック）
- [ ] 本番投入判断
