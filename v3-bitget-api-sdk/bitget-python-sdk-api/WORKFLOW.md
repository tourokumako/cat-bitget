...
※ 本番投入基準: **Replay NET $100/day 以上**（現在 -$1.4/day のため本番投入不可）
...
Step 2. 問題特定
  ...
  □ LONG / SHORT 別（同じ問題でも方向で性質が異なる）
  □ Priority 別（P2/P4/P23/P24 で原因が違う場合が多い）
  □ add_count 別（add=1 と add=2+ で損益構造が異なる）
  □ ADX 分布別（高ADX / 低ADX で挙動が異なる）
  □ 時間帯別（朝/日中/夜 で相場環境が異なる）
  □ パラメータ最適化はLONG/SHORTで別々に検討したか
      例: LONG_TP_PCT と SHORT_TP_PCT は別々に最適化できる

Step 2.5. 設計思想チェック ← STOP（提案前に必ず通過すること）
  「具体的な手段（TP幅・add回数等）は目的達成のために最適化する」が設計思想。
  手段そのものを禁じるのではなく、目的に近づくかを確認する。

  □ NET は改善するか（Replay または近似推定で確認）
  □ 平均保有時間が24h以内に収まるか（対象トレードの定義内か）
  □ 変更の目的を一言で説明できるか（「NET改善」「損失削減」等）
  ...
  理想形（狭いTP × 高TP率 × 速い回転）を維持したまま改善できる順に試す。
    ...
    ① TIME_EXIT 削減（損失を削る）  ← 理想形を維持したまま改善・最優先
    ② エントリーフィルター改善      ← 勝率を上げる
