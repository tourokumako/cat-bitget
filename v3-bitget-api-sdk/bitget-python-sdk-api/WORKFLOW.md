# WORKFLOW.md — V10 開発フロー（2026-04-07 更新）
...
| 項目 | 内容 |
|------|------|
| データ | 5m BTCUSDT 180日: `data/BTCUSDT-5m-2025-10-03_04-01_combined_180d.csv` |
| シミュレーター | `runner/replay_v10.py` |
| パラメータ | `config/cat_params_v10.json` |
| ポジションサイズ | 0.024 BTC |
| 手数料 | Maker 0.014% / Taker 0.042%（分岐点: 0.028% ≈ $24/BTC） |
| エントリー | close ± 0.01% 指値、TTL 2バー |
| add | なし（1エントリー1決済） |
| TIME_EXIT | **廃止**（TP/SLのみで決着） |

### シグナル（現在）

| 項目 | 値 |
|------|-----|
| ロング条件 | Stoch K/Dクロス上抜け（前足K < 20） |
| ショート条件 | Stoch K/Dクロス下抜け（前足K > 80） |
| TP | 0.0006（≈$50 @BTC$84k） |
| SL | 未設定（分析後に決める） |
| Stoch K/D/Slowing | 9/3/3 |

### Replay 結果履歴

#### 5m足（シグナル別 NET/日）

| シグナル | 件/日 | TP率 | NET/日 | 備考 |
|------|------|------|------|------|
| Stoch+RSI(30/70) TP=0.15% SL=0.30% | 11.5 | 67.7% | -$5.56 | GROSS初黒字だが手数料負け |
| Stoch OB/OS TP=0.0006 SL=0.003 | 23.0 | 83.7% | -$13.26 | SL広すぎで損失が大 |
| Stoch OB/OS + EMA200 TP=0.0006 SL=0.003 | 9.0 | 83.6% | -$5.28 | EMA200はOB/OSと逆方向・非採用 |

### 現状の課題（2026-04-07）

- シグナル自体のエッジが不明（分析未実施）
- SL_PCT が未設定のまま（0.003の古い値が残っている）
- EMA200フィルターがコードに残っている（revert 必要）

### 分析結果サマリー（2026-04-07）

| 条件 | 件/日 | TP率 | NET/日(0.024BTC) |
|------|------|------|------|
| 全体（Stoch OB/OS, TP=SL=$50） | 24.8 | 63.6% | -$6.56 |
| ADX≥30 | 7.1 | 67.3% | -$1.23 |
| ADX≥30 + K<10/K>90 | 0.9 | 73.3% | -$0.03 |

- LONG/SHORT 差なし（63.3% vs 63.9%）
- SL_FILLED の 84.5% は SL狭すぎによるバー内ノイズ踏み（シグナル方向は無関係）
- どのTP幅でも実TP率が必要TP率を 7〜10% 下回る（要次セッション検証）

### 次のタスク

- [ ] **要検証**: 「どのTP幅でも実TP率が必要TP率を7〜10%下回る」は本当か？
      → TP/SL非対称設計（SL固定・TP変化）での計算に誤りがないか確認する
- [ ] シグナルのエッジ不足が確定したら根本設計を見直す
- [ ] 90日データでの過学習チェック（採用後）
