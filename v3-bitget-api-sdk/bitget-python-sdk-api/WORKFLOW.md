# WORKFLOW.md — V10 開発フロー（2026-04-06 更新）
...
| 項目 | 内容 |
|------|------|
| データ | Binance 1m BTCUSDT（180日: `data/BTCUSDT-1m-binance-2026-04-06_180d.csv`） |
| シミュレーター | `runner/replay_1m.py` |
| パラメータ | `config/cat_params_v10.json` |
| ポジションサイズ | 0.024 BTC |
| 手数料 | Maker 0.014% / Taker 0.042% |
...
| モード | TP | SL | 用途 |
|--------|----|----|------|
| `mean_reversion` | 0.15% | 0.30% | BB外側+Stoch反転 |
| `trend_follow` | 0.30% | 0.15% | BBブレイク+Stoch順張り |
| `ema20` | 0.15% | 0.30% | 20EMA方向+Stochクロス |
| `rsi9` | 0.15% | 0.30% | RSI9 50ライン+Stochクロス |
...
### 各モードの Replay 結果（2026-04-06 時点）

| MODE | 件数/日 | TP率 | NET/日 | SL avg | TIME avg |
|------|---------|------|--------|--------|----------|
| ema20 | 17.7 | 42% | -$18.9 | -$6.64 | -$2.32 |
| rsi9 | 8.6 | 42% | -$8.1 | -$6.81 | -$2.13 |
| mean_reversion | 5.2 | 45% | -$5.0 | -$6.75 | -$2.32 |
| trend_follow | 49.0 | 23% | -$41.0 | -$3.61 | -$0.59 |

### 現状の課題（2026-04-06）

1. **TP率が全モード42〜45%** — フィルターを変えても変わらない。損益分岐(50%)未満
2. **TIME_EXIT が赤字の主因** — 逆行したまま最大20分保有するため avg -$2〜-$4/件
3. **SL=0.30% の損失が大きい** — SL発動時 avg -$6.75/件（TP +$2.50 の2.7倍）
4. **件数が少ない** — 最良の `ema20` でも 17.7件/日（目標100件/日に遠い）

### 次のタスク

- [ ] TIME_EXIT を廃止して TP/SL のみで決着させる検証
- [ ] シグナルに「エッジ」があるか統計的に確認
- [ ] エントリー条件の抜本的な見直し
...
3. ~~シミュレーター構築（1分足対応replay）~~ → `runner/replay_1m.py` 完成
4. 検証 → 本番投入 ← **現在ここ（未達）**
