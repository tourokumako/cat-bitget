#!/usr/bin/env python3
"""
runner/replay_v10.py — V10 スキャル戦略リプレイシミュレーター

使い方:
    cd v3-bitget-api-sdk/bitget-python-sdk-api
    .venv/bin/python3 runner/replay_v10.py /path/to/BTCUSDT-5m-...csv
...
def _detect_signals(df: pd.DataFrame, params: Dict) -> pd.DataFrame:
    stoch_ob = float(params.get("STOCH_OB", 80))
    stoch_os = float(params.get("STOCH_OS", 20))
    ...
    # クロス前の足が OS/OB 圏内にあった場合のみシグナル
    sig_long  = (k_p < stoch_os) & stoch_cross_up
    sig_short = (k_p > stoch_ob) & stoch_cross_down
