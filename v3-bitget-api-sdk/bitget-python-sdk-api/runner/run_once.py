...
def load_cat_params() -> dict:
    ...
    # STEP1: params が「ロードされた」証跡（重要キー列挙＋全体ハッシュ）
    try:
        import hashlib
        ...
        important = {k: _jsonable(d.get(k)) for k in IMPORTANT_PARAM_KEYS if k in d}
        important_missing = [k for k in IMPORTANT_PARAM_KEYS if k not in d]

        log("PARAMS_LOADED", {
            "path": str(p),
            "keys_count": len(keys_sorted),
            "keys_head": keys_sorted[:10],
            "sha256": sha256,
            "important": important,
...
def classify_external_exit(pos: Dict[str, Any], last_price: float) -> str:
    """
    外部決済（TP/SL plan 約定など）で pos が消えたケースを runner 側 Exit 理由へ再分類。
    論理一致：TP / SL のみ（RSI/TIME は外部決済からは判断不能なため対象外）
    """
    ...
    # まずは tp/sl を跨いだかで判定（最も筋が通る）
    if side == "LONG":
        if tp_f is not None and last_price >= tp_f:
            return "TP"
        if sl_f is not None and last_price <= sl_f:
            return "SL"
    elif side == "SHORT":
        if tp_f is not None and last_price <= tp_f:
            return "TP"
    ...
    # tp/sl が欠けてる/価格が取れない等でも落とさず、価格方向で決め打ち（論理一致）
    if side == "LONG":
        if entry_f == entry_f:
            return "TP" if last_price >= entry_f else "SL"
        return "SL"
    if side == "SHORT":
        if entry_f == entry_f:
            return "TP" if last_price <= entry_f else "SL"
...
def main() -> None:
    ...
    log("BOOT", {"run_id": run_id, "symbol": SYMBOL, "productType": PRODUCT_TYPE, "file": __file__})
    ...
    # run identifier (per process)
    try:
        _RUN_SEQ = _RUN_SEQ + 1  # type: ignore[name-defined]
    except Exception:
        _RUN_SEQ = 1
    log("RUN_START", {"run_seq": _RUN_SEQ})

    log("RUN_ID", {"run_id": run_id})
    log("PRECHECK", {"pos_count": pos_n, "pending_pl_count": pend_n})

    log("PRECHECK_OK")

    # --- [PATCH] 外部決済（TP/SL plan約定）を Exit として補足：return前に必ず処理する ---
    # 条件：前runの open_position が存在 & 今runで pos_n==0
    try:
        op = read_open_position()
        if isinstance(op, dict) and int(pos_n) == 0 and op.get("entry_time"):

            # last_price を最小コストで取得（tickerのみ）
            tkr = adp.get_ticker(PRODUCT_TYPE, SYMBOL)
            t0 = (tkr.get("data") or [])
            t0 = t0[0] if isinstance(t0, list) and t0 else {}
            try:
                last_price = float(str(t0.get("lastPr") or t0.get("markPrice")))
            except Exception:
                last_price = float("nan")

            exit_reason = classify_external_exit(op, last_price)

            log("EXIT_DECIDED", {
                "exit_reason": exit_reason,
                "path": "external_plan_filled",
                "pos_count": int(pos_n),
                "pending_pl_count": int(pend_n),
                "last_price": last_price,
                "side": op.get("side"),
                "entry_priority": op.get("entry_priority"),
                "tp": op.get("tp"),
                "sl": op.get("sl"),
            })

            # decision を EXIT として残す（CLOSEは送らない：取引所側で既にクローズ済み）
            try:
                write_json(state_path("decision.json"), {
                    "action": "EXIT",
                    "reason": f"runner_exit:{exit_reason}(external_plan_filled)",
                    "side": op.get("side"),
                    "entry_priority": op.get("entry_priority"),
                })
                log("DECISION_WRITTEN", {"path": state_path("decision.json")})
            except Exception:
                pass

            # 二重計上防止：ローカル状態を消す
            delete_open_position()
            log("OPEN_POSITION_DELETED", {"path": state_path("open_position.json")})

            log("END", {"result": "EXIT_RECONCILED"})
            return
    except Exception:
        pass
    ...
    # --- [PATCH] 外部決済（TP/SL plan約定）を Exit として補足 ---
    # 条件：前runの open_position がある & 今runの PRECHECK で pos_n==0
    op = read_open_position()
    if isinstance(op, dict) and int(pos_n) == 0 and op.get("entry_time"):
        # last_price は ticker の lastPr を優先（無ければ markPrice）
        try:
            last_price = float(str(t0.get("lastPr") or t0.get("markPrice")))
        except Exception:
            last_price = float("nan")
        ...
        # decision.json も EXIT として記録（後段で上書きされてもログが残ればOK）
        try:
            write_json(state_path("decision.json"), {
                "action": "EXIT",
                "reason": f"runner_exit:{exit_reason}(external_plan_filled)",
        ...
        # 二重計上防止：open_position を消す（次runで再度 Exit 判定しない）
        delete_open_position()
        log("OPEN_POSITION_DELETED", {"path": state_path("open_position.json")})
    ...
    # F1: 対象足（candle_last）の ts/OHLC をログに残す（APIレスポンスtsではない）
        # ★A: strategy 入力の正本に params/trace_flags を含める（SNAPSHOT保存前）
    market_snapshot["params"] = params if isinstance(params, dict) else {}

    log("CANDLE_USED", {"candle_last": market_snapshot.get("candle_last")})
    log("TARGET_TS", {"target_ts": (market_snapshot.get("candle_last") or {}).get("ts")})
    log("GATE", {"entry_ok": True, "pos_count": pos_n, "MAX_POSITIONS": int(params.get("MAX_POSITIONS", 5)), "tag": "F1"})
    ...
    # F1/F2用：snapshot を上書きで失わないようにアーカイブ保存（市場依存ゼロで後から replay するため）
    try:
        import pathlib
        from time import time as _time

        snap_dir = pathlib.Path(state_path("snapshots"))
        snap_dir.mkdir(parents=True, exist_ok=True)

        ts = (market_snapshot.get("candle_last") or {}).get("ts")
        ts_str = str(ts) if ts is not None else "no_ts"

        # 同一5分足(ts)の連打保存を防止（F1の網羅効率を上げる）
        last_ts_path = snap_dir / "last_target_ts.txt"
        last_ts = None
        try:
            if last_ts_path.exists():
                last_ts = last_ts_path.read_text(encoding="utf-8").strip()
        except Exception:
            last_ts = None

        if (ts_str != "no_ts") and (last_ts == ts_str):
            log("SNAPSHOT_ARCHIVE_SKIPPED", {"reason": "same_target_ts", "ts": ts_str})
        else:
            base = f"{ts_str}_market_snapshot"
            out_path = snap_dir / f"{base}.json"
            if out_path.exists():
                out_path = snap_dir / f"{base}_{int(_time() * 1000)}.json"

            write_json(out_path, market_snapshot)

            try:
                last_ts_path.write_text(ts_str, encoding="utf-8")
            except Exception:
                pass

            log("SNAPSHOT_ARCHIVED", {"path": str(out_path)})
    except Exception as _e:
        log("SNAPSHOT_ARCHIVE_FAIL", {"err": str(_e)})
    ...
    decision = None
    try:
        override_exists = Path(override_path).exists()
        log("OVERRIDE_STATUS", {
    ...
    # F1: fired priority の material が「あるか/何が入ってるか」だけ必ず出す（設計判断は後）
    mat = decision.get("material") if isinstance(decision, dict) else None
    mat_dict = mat if isinstance(mat, dict) else {}
    log("MAT_FIRED", {
        "entry_priority": (decision.get("entry_priority") if isinstance(decision, dict) else None),
        "has_material": isinstance(mat, dict),
        "material_keys": sorted(list(mat_dict.keys())),
    })

    # F1: priority非依存の最低限材料（CAT側と突合できるコア）
    ind = market_snapshot.get("indicators") if isinstance(market_snapshot.get("indicators"), dict) else {}
    c = market_snapshot.get("candle_last") if isinstance(market_snapshot.get("candle_last"), dict) else {}
    log("MATERIAL_CORE", {
        "target_ts": c.get("ts"),
        "ohlc": {"open": c.get("open"), "high": c.get("high"), "low": c.get("low"), "close": c.get("close")},

        # gate
        "entry_ok": market_snapshot.get("entry_ok"),
        "pos_count": market_snapshot.get("pos_count"),
        "MAX_POSITIONS": market_snapshot.get("MAX_POSITIONS"),

        # indicators（存在するものだけ）
        "ema_20": ind.get("ema_20"),
        "ema_5": ind.get("ema_5"),
        "adx": ind.get("adx"),
        "bb_mid_slope": ind.get("bb_mid_slope"),
        "bb_width_now": ind.get("bb_width_now"),
        "bb_width_prev": ind.get("bb_width_prev"),
        "bb_lower": ind.get("bb_lower"),
        "bb_upper": ind.get("bb_upper"),
        "rci7_now": ind.get("rci7_now"),
        "rci7_prev": ind.get("rci7_prev"),
        "rci9_now": ind.get("rci9_now"),
        "rci9_prev": ind.get("rci9_prev"),
        "bullish": ind.get("bullish"),
    })

    # F1: material proof for fired priority (P3)
    if isinstance(decision, dict) and str(decision.get("entry_priority")) == "3":
        mat = decision.get("material") if isinstance(decision, dict) else None
        mat = mat if isinstance(mat, dict) else {}
        rci_ctx = mat.get("rci_ctx") if isinstance(mat.get("rci_ctx"), dict) else {}
        log("MAT_P3", {
            "rci7_prev": rci_ctx.get("rci7_prev"),
            "rci9_prev": rci_ctx.get("rci9_prev"),
            "rci7_now": rci_ctx.get("rci7_now"),
            "rci9_now": rci_ctx.get("rci9_now"),
            "bb_mid_slope": mat.get("bb_mid_slope"),
            "missing": {
                "has_material": isinstance(decision.get("material"), dict) if isinstance(decision, dict) else False,
                "has_rci_ctx": isinstance(mat.get("rci_ctx"), dict),
                "has_bb_mid_slope": ("bb_mid_slope" in mat),
            }
        })

    # F1: material proof for fired priority (P23)
    if isinstance(decision, dict) and str(decision.get("entry_priority")) == "23":
        mat = decision.get("material") if isinstance(decision, dict) else None
        mat = mat if isinstance(mat, dict) else {}
        log("MAT_P23", {
            "stoch_k": mat.get("stoch_k"),
            "stoch_d": mat.get("stoch_d"),
            "gap": mat.get("gap"),
            "bearish": mat.get("bearish"),
            "ENABLE_P23_SHORT": mat.get("ENABLE_P23_SHORT"),
            "missing": {
                "has_material": isinstance(decision.get("material"), dict) if isinstance(decision, dict) else False,
                "has_stoch_k": ("stoch_k" in mat),
                "has_stoch_d": ("stoch_d" in mat),
                "has_gap": ("gap" in mat),
                "has_bearish": ("bearish" in mat),
                "has_enable": ("ENABLE_P23_SHORT" in mat),
            }
        })
...
# ====== 修正後（run_once.py：decision確定直後に追加）======
    ...
    # --- ACTION ---
    if action == "NOOP":
        # F1: NOOP reason (single-line)
        log("NOOP_REASON", {
            "reason": (decision.get("reason") if isinstance(decision, dict) else None),
            "pos_count": pos_n,
            "result": ("HOLDING" if pos_n != 0 else "NOOP"),
        })
    ...
    # C: MAX_POSITIONS に達している場合は新規ENTRYを抑止（multi-position対応）
    if action == "ENTER":
        try:
            max_pos = int(load_cat_params().get("MAX_POSITIONS", 1))
        except Exception:
            max_pos = 1
        if pos_n >= max_pos:
            log("ENTRY_GUARD_NOOP", {"reason": "pos>=MAX_POSITIONS", "pos_n": pos_n, "MAX_POSITIONS": max_pos})
            log("END", {"result": "ENTRY_SKIPPED_MAX_POSITIONS"})
            return

    if action == "ENTER":
        pass  # TODO: implement entry logic or remove this block if not needed
        # 必須項目チェック（不足はSTOP）
    ...
    if action == "ENTER":
        ...
        # STEP1: params が「実際に使われた」証跡（ENTER の size 決定に直結）
        try:
            log("PARAMS_USED", {
                "context": "ENTER",
                "key": "POSITION_SIZE_BTC",
                "present": ("POSITION_SIZE_BTC" in params),
                "raw_value": params.get("POSITION_SIZE_BTC", None),
            })
        except Exception:
            pass
        ...
        # safety: multi-position 対応（MAX_POSITIONS まで許可）
        try:
            max_pos = int(load_cat_params().get("MAX_POSITIONS", 1))
        except Exception:
            max_pos = 1
        ...
        if action == "ENTER":
            # --- ADD cap guard (CAT: add_count is state; source of truth = open_position.json) ---
            try:
                op0 = read_open_position()
            except Exception:
                op0 = None
            ...
            is_add_attempt = (
                isinstance(op0, dict)
                and int(pos_n) > 0
                and str(op0.get("side")).upper() == str(side).upper()
            )

            if is_add_attempt:
                try:
                    add_count = int(op0.get("add_count", 0))
                except Exception:
                    add_count = 0

                cap = None
                if entry_priority in (1, 2):
                    cap = 1
                elif entry_priority in (4, 22):
                    cap = 5

                if cap is not None and add_count >= cap:
                    log("ENTRY_GUARD_NOOP", {
                        "reason": "add_count>=cap",
                        "pos_n": int(pos_n),
                        "side": str(side),
                    ...
                    log("END", {"result": "ENTRY_SKIPPED_ADD_CAP"})
                    return

            log("ENTRY_SEND", {"clientOid": coid, "side": side, "size": size})


            # STEP1: 送信する注文パラメータの証跡（ENTRY）
            log("ORDER_PARAMS", {
                "kind": "ENTRY",
                "clientOid": coid,
                "side": side,          # LONG/SHORT（判断側）
                "size": size,          # Bitget注文サイズ（BTC文字列）
                "open_side": open_side,  # buy/sell（Bitget側）
                "hold_side": hold_side,  # long/short（Bitget側）
            })
...
# ====== 修正後（run_once.py：ENTER 成功時に open_position を保存）======
        ...
        try:
            ...
            # read previous open_position for stateful fields (e.g., add_count, first entry_time)
            try:
                prev = read_open_position()
            except Exception:
                prev = None

            is_add = (
                isinstance(prev, dict)
                and str(prev.get("side")).upper() == str(side).upper()
                and prev.get("entry_time") is not None
            )

            # keep first entry_time on ADD (do NOT reset holding clock)
            entry_time = prev.get("entry_time") if is_add else now_ts

            prev_add = 0
            if is_add:
                try:
                    prev_add = int(prev.get("add_count", 0))
                except Exception:
                    prev_add = 0

            # increment add_count only when ADD is actually done (we are in ENTER_DONE path here)
            add_count = (prev_add + 1) if is_add else prev_add
    ...
    if action == "EXIT":
        ...
        # STEP1: 送信する注文パラメータの証跡（CLOSE）
        log("ORDER_PARAMS", {
            "kind": "CLOSE",
            "clientOid": payload.get("clientOid") if isinstance(payload, dict) else None,
            "size": payload.get("size") if isinstance(payload, dict) else None,
            "side": payload.get("side") if isinstance(payload, dict) else None,
            "holdSide": payload.get("holdSide") if isinstance(payload, dict) else None,
        ...
        try:
            close_r = adp.close_market_order(
                symbol=SYMBOL,
                product_type=PRODUCT_TYPE,
                margin_mode="isolated",
                margin_coin=MARGIN_COIN,
            ...
            oid = None
            try:
                oid = (close_r.get("data") or {}).get("orderId")
            except Exception:
                oid = None
            log("CLOSE_RESP", {"clientOid": payload["clientOid"], "orderId": oid})
        ...
        # CLOSE_OK 直後の実ポジ再照会（F1監査の必須証跡）
        # get_single_position() は戻り型差で falsy になり得るので、
        # ログに実在している get_positions() ベースで判定する
        try:
            ps3 = adp.get_positions(PRODUCT_TYPE, MARGIN_COIN) or []
            # BTCUSDT + holdSide の該当を拾う（無ければ pos_exists=False）
            p3 = None
            for p in ps3:
                try:
                    if p.get("symbol") == SYMBOL and p.get("holdSide") == hold_side:
                        p3 = p
                        break
                except Exception:
                    continue
            ...
            if not p3:
                log("CLOSE_VERIFY", {"pos_exists": False, "positions_n": len(ps3)})
            else:
                total = p3.get("total")
                avail = p3.get("available")
                log("CLOSE_VERIFY", {
                    "pos_exists": True,
                    "positions_n": len(ps3),
                    "total": total,
                ...
                # まだ残ってるなら EXIT_DONE にしない（open_positionも消さない）
                try:
                ...
                if (t > 0.0) or (a > 0.0):
                    log("END", {"result": "EXIT_PENDING_POS_STILL_EXISTS"})
                    return

        except Exception as e:
            log("CLOSE_VERIFY_FAIL", {"error": str(e)})
            log("END", {"result": "EXIT_PENDING_VERIFY_FAIL"})
            return
