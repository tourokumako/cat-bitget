...
class Client(object):
    ...
    def _request(self, method, request_path, params, cursor=False):
        ...
        paper_trading = bool(getattr(self, "PAPER_TRADING", False))
        header = utils.get_header(self.API_KEY, sign, timestamp, self.PASSPHRASE, paper_trading=paper_trading)
    ...
    def _get_timestamp(self):
        ...
        if response.status_code == 200:
            j = response.json()
            return int(j.get("data", {}).get("serverTime") or j.get("timestamp") or j.get("data", {}).get("timestamp"))
